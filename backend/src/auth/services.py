...
def authenticate_user(email: str, password: str, db: Session) -> schemas.User:
    userDB = get_user_from_db_by_email(email, db)
    if not userDB:
        raise AUTHENTICATION_EXCEPTION
    hashPassword = str(userDB.hashed_password)
    if not bcrypt_context.verify(password, hashPassword):
        raise AUTHENTICATION_EXCEPTION
    return userDB


def create_access_token(
    email: str, user_id: int, expires_delta: timedelta = timedelta(minutes=20)
):
...
def add_new_user_to_db(email: str, password: str, db: Session):
    ...
    userDB = models.User(email=email, hashed_password=_hash_password)
    db.add(userDB)
    db.commit()
    db.refresh(userDB)
    return userDB
...
def decode_access_token(token: Annotated[str, Depends(oauth2_bearer)]):
    try:
        ...
        if email is None or user_id is None:
            raise AUTHENTICATION_EXCEPTION
        user = schemas.User(email=email, id=user_id)
        return user
    except ExpiredSignatureError:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Access Token Expires"
        )
    except JWTError:
        raise AUTHENTICATION_EXCEPTION
...
def get_current_user(
    user: Annotated[schemas.User, Depends(decode_access_token)],
