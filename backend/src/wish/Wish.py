...
from .services import add_new_wish_to_db, edit_wish_in_db, get_user_wishes_from_db, hide_wish_in_db, remove_wish_from_db, unhide_wish_in_db
...
@router.put('/{id}', response_model=schemas.Wish)
def update_wishes(db: db_dependency, user: user_dependency, wish: schemas.WishEdit, id: int):
    return edit_wish_in_db(db=db, id=id, user_id=user.id, **wish.model_dump())
